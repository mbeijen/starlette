...
The test client exposes the same interface as any other `httpxyz` session.
In particular, note that the calls to make a request are just standard
function calls, not awaitables.

You can use any of `httpxyz` standard API, such as authentication, session
...
For more information you can check the `httpxyz` [documentation](https://httpxyz.org/advanced/).
...
The `httpxyz` library will be used to build the initial handshake, meaning you
...
* `.websocket_connect(url, subprotocols=None, **options)` - Takes the same set of arguments as `httpxyz.get()`.
...
The simplest way to work around this is to just make your entire test async and use an async client, like [httpxyz.AsyncClient].

Here is an example of such a test:

```python
from httpxyz import AsyncClient, ASGITransport
...
[httpxyz.AsyncClient]: https://httpxyz.org/advanced/#calling-into-python-web-apps
